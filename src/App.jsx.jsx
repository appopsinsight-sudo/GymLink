...
const MatchesScreen=({confirmedMatches,ltConfirmedMatches=[],initialChat,onChatOpened,onCancel})=>{
  ...
  const[cancelDone,setCancelDone]=useState(null); // holds the cancelled user for rating
  ...
  if(cancelTarget)return <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(4px)"}} onClick={()=>setCancelTarget(null)}/>
    <div onClick={e=>e.stopPropagation()} style={{position:"relative",width:"100%",maxWidth:420,background:T.bg,borderRadius:T.radiusMd,padding:24,border:`1px solid ${T.border}`}}>
      ...
      <div style={{display:"flex",gap:10}}>
        ...
        <Btn onClick={()=>{if(onCancel)onCancel(cancelTarget.sessionKey);setCancelDone(cancelTarget);setCancelTarget(null);setCancelMsg("")}} variant="danger" size="lg" style={{flex:1}}><X size={16}/> Cancel Session</Btn>
      </div>
    </div>
  </div>;

  // Post-cancel: optional anonymous rating
  if(cancelDone&&!ratingSkipped)return <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.7)",backdropFilter:"blur(4px)"}}/>
    <div onClick={e=>e.stopPropagation()} style={{position:"relative",width:"100%",maxWidth:420,background:T.bg,borderRadius:T.radiusMd,padding:24,border:`1px solid ${T.border}`,textAlign:"center"}}>
      <AlertTriangle size={36} color={T.warning} style={{marginBottom:12}}/>
      <h3 style={{fontSize:18,fontWeight:700,marginBottom:8}}>Session Cancelled</h3>
      <p style={{fontSize:13,color:T.textMuted,marginBottom:20}}>Your session with {cancelDone.name} has been cancelled. Would you like to leave an anonymous reliability rating? This is optional and will only be visible after 25 days.</p>
      <div style={{display:"flex",gap:10,marginBottom:12}}>
        <Btn onClick={()=>{setRatingSkipped(true);setCancelDone(null)}} variant="secondary" size="lg" style={{flex:1}}>Skip</Btn>
        <Btn onClick={()=>{setRatingSkipped(true);setCancelDone(null)}} variant="warning" size="lg" style={{flex:1}}><Flag size={14}/> Rate Reliability</Btn>
      </div>
      <p style={{fontSize:11,color:T.textDim}}>Anonymous — the other person won't know who rated them</p>
    </div>
  </div>;
