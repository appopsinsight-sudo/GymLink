...
const DiscoverScreen=({userProfile,sentRequests,onSendRequest,confirmedMatches,ltSentRequests=[],setLtSentRequests,ltConfirmedMatches=[],setLtConfirmedMatches})=>{
  ...
  const searchByLoc=async()=>{if(!navigator.geolocation){alert("Location not supported");return}navigator.geolocation.getCurrentPosition(async(pos)=>{const results=await searchGymsNearLocation(pos.coords.latitude,pos.coords.longitude);setGymR(results);setSearched(true);setShowPostcode(true)},()=>alert("Location access denied"),{enableHighAccuracy:true})};
  ...
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
    ...
    {mode&&<div className="fade-in" style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {homeGymData?<div>
        ...
        {!showPostcode?<div style={{display:"flex",gap:8}}><button onClick={()=>setShowPostcode(true)} style={{background:"none",border:"none",fontSize:13,color:T.primaryLight,cursor:"pointer",padding:0,fontFamily:T.font}}>Search other gyms by postcode</button><span style={{color:T.border}}>·</span><button onClick={searchByLoc} style={{background:"none",border:"none",fontSize:13,color:T.accent,cursor:"pointer",padding:0,fontFamily:T.font}}>Use my location</button></div>:
      ...
      <div><label style={{display:"block",fontSize:13,fontWeight:500,color:T.textMuted,marginBottom:6}}>Find gym by postcode</label><div style={{display:"flex",gap:10}}><Input placeholder="e.g. EC1V" value={sPC} onChange={e=>setSPC(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchGyms()} containerStyle={{flex:1}}/><Btn onClick={searchGyms} variant="secondary"><Search size={18}/></Btn></div><Btn onClick={searchByLoc} variant="outline" size="sm" style={{width:"100%",marginTop:8}}><MapPin size={14}/> Use my location</Btn>
