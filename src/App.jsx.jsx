...
const TrainersScreen=({userProfile})=>{
  ...
  const handleTrainerEnquiry=(type,data)=>{setEnquirySent(type==="session"?`Session request sent to ${trainerProfile?.name}`:`Enquiry sent to ${trainerProfile?.name}`);setTimeout(()=>setEnquirySent(null),3000);setTrainerProfile(null)};
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
