...
const OnboardingFlow=({onComplete})=>{
  ...
  const rc=()=>{switch(cur){
    ...
    case"gym":case"gyms_trainer":return <div key={ak} className="fade-in" style={{maxWidth:480,width:"100%",margin:"0 auto"}}><SI icon={MapPin} color={T.success} title={cur==="gym"?"Find Your Gym":"Select Gyms"}/><div style={{display:"flex",gap:10,marginBottom:20}}><Input placeholder="e.g. EC1V" value={p.postcode} onChange={e=>up("postcode",e.target.value)} onKeyDown={e=>e.key==="Enter"&&sG()} containerStyle={{flex:1}}/><Btn onClick={sG} variant="secondary" style={{flexShrink:0}}><Search size={18}/></Btn></div>{sp2&&gymR.length>0&&gymList(gymR)}{sp2&&!gymR.length&&<p style={{textAlign:"center",color:T.textDim,fontSize:14,marginBottom:20}}>No gyms found.</p>}{err.gym&&<p style={{fontSize:12,color:T.danger,marginBottom:8}}>{err.gym}</p>}{err.hg&&<p style={{fontSize:12,color:T.danger,marginBottom:8}}>{err.hg}</p>}<Btn onClick={hN} size="lg" style={{width:"100%"}} disabled={!p.gyms.length}>Continue <ChevronRight size={18}/></Btn></div>;
...
const DiscoverScreen=({userProfile,sentRequests,onSendRequest,confirmedMatches,ltSentRequests=[],setLtSentRequests,ltConfirmedMatches=[],setLtConfirmedMatches})=>{
  ...
  const[sPC,setSPC]=useState(userProfile.postcode||"");
  ...
  const searchGyms=async()=>{const results=await searchGymsNearPostcode(sPC);setGymR(results);setSearched(true)};
  ...
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
    ...
    {mode&&<div className="fade-in" style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {/* Home gym default — postcode optional */}
      {homeGymData?<div>
        ...
        {!showPostcode?<button onClick={()=>setShowPostcode(true)} style={{background:"none",border:"none",fontSize:13,color:T.primaryLight,cursor:"pointer",padding:0,fontFamily:T.font}}>Search other gyms by postcode</button>:
        <div><div style={{display:"flex",gap:10,marginTop:8}}><Input placeholder="e.g. EC1V" value={sPC} onChange={e=>setSPC(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchGyms()} containerStyle={{flex:1}}/><Btn onClick={searchGyms} variant="secondary"><Search size={18}/></Btn></div>
        {searched&&gymR.length>0&&<div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:160,overflowY:"auto",marginTop:8}}>{gymR.map(g=><Card key={g.id} onClick={()=>setSGym(g.id)} style={{padding:12,borderColor:sGym===g.id?T.primary:T.border,background:sGym===g.id?T.primary+"08":T.bgCard,display:"flex",alignItems:"center",gap:10}}><div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${sGym===g.id?T.primary:T.border}`,background:sGym===g.id?T.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{sGym===g.id&&<Check size={10} color="#fff"/>}</div><div style={{flex:1}}><p style={{fontWeight:600,fontSize:13}}>{g.name}</p><p style={{fontSize:11,color:T.textDim}}>{g.address}</p></div></Card>)}</div>}</div>}
      </div>:
      <div><label style={{display:"block",fontSize:13,fontWeight:500,color:T.textMuted,marginBottom:6}}>Find gym by postcode</label><div style={{display:"flex",gap:10}}><Input placeholder="e.g. EC1V" value={sPC} onChange={e=>setSPC(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchGyms()} containerStyle={{flex:1}}/><Btn onClick={searchGyms} variant="secondary"><Search size={18}/></Btn></div>
      {searched&&gymR.length>0&&<div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:180,overflowY:"auto",marginTop:8}}>{gymR.map(g=><Card key={g.id} onClick={()=>setSGym(g.id)} style={{padding:12,borderColor:sGym===g.id?T.primary:T.border,background:sGym===g.id?T.primary+"08":T.bgCard,display:"flex",alignItems:"center",gap:10}}><div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${sGym===g.id?T.primary:T.border}`,background:sGym===g.id?T.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{sGym===g.id&&<Check size={10} color="#fff"/>}</div><div style={{flex:1}}><p style={{fontWeight:600,fontSize:13}}>{g.name}</p><p style={{fontSize:11,color:T.textDim}}>{g.address}</p></div></Card>)}</div>}</div>}
