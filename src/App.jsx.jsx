...
const DiscoverScreen=({userProfile,sentRequests,onSendRequest,confirmedMatches,ltSentRequests=[],setLtSentRequests,ltConfirmedMatches=[],setLtConfirmedMatches})=>{
  ...
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
    ...
    <div style={{display:"flex",gap:12,marginBottom:24}}>
      <Card onClick={()=>{setMode("today");setLtSearched(false)}} style={{flex:1,padding:18,textAlign:"center",borderColor:mode==="today"?T.primary:T.border,background:mode==="today"?T.primary+"08":T.bgCard}}><Zap size={24} color={mode==="today"?T.primary:T.textDim} style={{margin:"0 auto 8px"}}/><p style={{fontWeight:700,fontSize:13,color:mode==="today"?T.text:T.textMuted}}>Today</p></Card>
      <Card onClick={()=>{setMode("future");setLtSearched(false)}} style={{flex:1,padding:18,textAlign:"center",borderColor:mode==="future"?T.primary:T.border,background:mode==="future"?T.primary+"08":T.bgCard}}><Calendar size={24} color={mode==="future"?T.primary:T.textDim} style={{margin:"0 auto 8px"}}/><p style={{fontWeight:700,fontSize:13,color:mode==="future"?T.text:T.textMuted}}>Future Date</p></Card>
      <Card onClick={()=>{setMode("longterm");setLtSearched(false)}} style={{flex:1,padding:18,textAlign:"center",borderColor:mode==="longterm"?T.accent:T.border,background:mode==="longterm"?T.accent+"08":T.bgCard}}><Users size={24} color={mode==="longterm"?T.accent:T.textDim} style={{margin:"0 auto 8px"}}/><p style={{fontWeight:700,fontSize:13,color:mode==="longterm"?T.text:T.textMuted}}>Long-term</p></Card>
    </div>
...
const MatchesScreen=({confirmedMatches,ltConfirmedMatches=[],initialChat,onChatOpened,onCancel})=>{
  ...
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
    ...
    {preview&&<ProfilePreview user={preview} onConnect={()=>setPreview(null)} onPass={()=>setPreview(null)} onClose={()=>setPreview(null)} connected={true}/>}<div style={{marginBottom:24}}><h1 style={{fontSize:26,fontWeight:800}}>Matches & Connections</h1><p style={{color:T.textMuted,fontSize:14,marginTop:4}}>Confirmed partners — chat is unlocked</p></div>
    ...
    if(proposeAlt)return <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{position:"relative",width:"100%",maxWidth:440,maxHeight:"85vh",overflowY:"auto",background:T.bg,borderRadius:T.radiusMd,padding:24,border:`1px solid ${T.border}`}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <Btn onClick={()=>{handleDeclineT(proposeAlt.id);setProposeAlt(null);setAltDate("");setAltTime("");setAltMsg("")}} variant="primary" size="lg" style={{flex:1}} disabled={!altDate||!altTime}><Send size={16}/> Send Proposal</Btn>
        </div>
      </div>
    </div>;
  ...
  if(subView==="postGroup"||subView==="editGroup"){
    ...
    return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16,marginBottom:24}}>
        ...
        {gyms.length>0?<div style={{display:"flex",flexDirection:"column",gap:8}}>{gyms.map(g=><Card key={g.id} onClick={()=>setGGym(g.id)} style={{padding:12,display:"flex",alignItems:"center",gap:10,borderColor:gGym===g.id?T.primary:T.border,background:gGym===g.id?T.primary+"08":T.bgCard,cursor:"pointer"}}>
        ...
        </Card>)}</div>:<Input label="" placeholder="Enter gym name" value={gGym} onChange={e=>setGGym(e.target.value)}/>}</div>
      </div>
      <Btn onClick={()=>{
...
export default function GymLink(){
  ...
  useEffect(()=>{try{localStorage.setItem("gl_profile",JSON.stringify(userProfile));localStorage.setItem("gl_screen",screen);localStorage.setItem("gl_tab",activeTab)}catch(e){}},[userProfile,screen,activeTab]);
  ...
  const handleLogout=()=>{setScreen("login");setActiveTab("discover");setSentRequests([]);setIncomingRequests([]);setConfirmedMatches([]);setLtSentRequests([]);setLtConfirmedMatches([]);setTrainerReqs(MOCK_TRAINER_REQUESTS);setAcceptedBookings([]);setTrainerBootcamps(MOCK_BOOTCAMPS.filter(b=>b.trainerId==="t1"));setTrainerGroups(MOCK_GROUP.filter(g=>g.trainerId==="t1"));window._glTrainerReqs=null;window._glAccepted=null;try{localStorage.setItem("gl_screen","login")}catch(e){}};
  const handleDeleteAccount=()=>{setScreen("onboarding");setActiveTab("discover");setUserProfile({});setSentRequests([]);setIncomingRequests([]);setConfirmedMatches([]);setLtSentRequests([]);setLtConfirmedMatches([]);setTrainerReqs(MOCK_TRAINER_REQUESTS);setAcceptedBookings([]);setTrainerBootcamps(MOCK_BOOTCAMPS.filter(b=>b.trainerId==="t1"));setTrainerGroups(MOCK_GROUP.filter(g=>g.trainerId==="t1"));window._glTrainerReqs=null;window._glAccepted=null;try{localStorage.removeItem("gl_profile");localStorage.removeItem("gl_screen");localStorage.removeItem("gl_tab")}catch(e){}};
  ...
  if(screen==="login"){
    const savedName=userProfile.name||userProfile.firstName||"";
    return <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:400,textAlign:"center"}}>
        <div style={{width:80,height:80,borderRadius:24,background:T.gradient,margin:"0 auto 24px",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 40px ${T.primaryGlow}`}}><Dumbbell size={36} color="#fff"/></div>
        <h1 style={{fontSize:28,fontWeight:800,marginBottom:8}}>Welcome Back{savedName?`, ${savedName}`:""}!</h1>
        <p style={{color:T.textMuted,fontSize:14,marginBottom:32}}>Your profile is saved and ready to go.</p>
        {savedName?<div style={{display:"flex",flexDirection:"column",gap:12}}>
          <Card style={{padding:18,display:"flex",alignItems:"center",gap:14}}>
            <Avatar name={savedName} size={50} verified={true}/>
            <div style={{flex:1,textAlign:"left"}}><p style={{fontWeight:600,fontSize:15}}>{userProfile.name}</p><p style={{fontSize:12,color:T.textDim}}>{userProfile.role==="trainer"?"Personal Trainer":"Gym Member"}</p></div>
          </Card>
          <Btn onClick={()=>{setScreen("app");setActiveTab(userProfile.role==="trainer"?"dashboard":"discover");try{localStorage.setItem("gl_screen","app")}catch(e){}}} size="lg" style={{width:"100%"}}><Zap size={18}/> Continue as {userProfile.name}</Btn>
          <Btn onClick={handleDeleteAccount} variant="ghost" size="sm" style={{marginTop:8}}><Trash2 size={14}/> Sign out & delete account</Btn>
        </div>:<div>
          <p style={{color:T.textDim,marginBottom:20}}>No saved profile found.</p>
          <Btn onClick={()=>{setScreen("onboarding");try{localStorage.setItem("gl_screen","onboarding")}catch(e){}}} size="lg" style={{width:"100%"}}>Create Account</Btn>
        </div>}
      </div>
    </div>;
  }
