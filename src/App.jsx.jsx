...
const RequestsScreen=({incomingRequests,onAccept,onDecline,sentRequests,role,confirmedMatches=[],trainerReqs:tReqs,setTrainerReqs:setTReqs,acceptedBookings,setAcceptedBookings,trainerBootcamps,setTrainerBootcamps,trainerGroups,setTrainerGroups})=>{
  ...
  return <div className="fade-in" style={{padding:"16px 20px 100px"}}>{preview&&<ProfilePreview user={preview} onConnect={()=>setPreview(null)} onPass={()=>setPreview(null)} onClose={()=>setPreview(null)} connected={true}/>}<div style={{marginBottom:20}}><h1 style={{fontSize:26,fontWeight:800}}>Requests</h1><p style={{color:T.textMuted,fontSize:14,marginTop:4}}>Gym buddy match requests</p></div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>{sentRequests.map((u,i)=>{const gy=u.gymId?GYMS_DB.find(g=>g.id===u.gymId):null;const isConfirmed=confirmedMatches.some(r=>r.sessionKey===u.sessionKey);return <Card key={u.sessionKey||u.id+i} style={{padding:18}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:8}}><Avatar name={u.name} size={44} verified={true}/><div style={{flex:1}}><p style={{fontWeight:600,fontSize:14}}>{u.name}</p><p style={{fontSize:12,color:T.textDim}}>{u.fitnessLevel}</p></div><Badge color={isConfirmed?T.success:T.warning}>{isConfirmed?"Accepted":"Pending"}</Badge></div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
      ...
      {isConfirmed&&<Btn onClick={()=>setChat(u)} variant="primary" size="sm" style={{width:"100%"}}><MessageCircle size={14}/> Message</Btn>}
    </Card>})}</div>)}
  </div>;
};
