  ...
  const getMatches=()=>{
    ...
    for(const tw of timeWindows){
      const tier=MOCK_USERS.filter(u=>{
        if(!sGyms.includes(u.gym))return false;
...
const ProfileScreen=({userProfile,setUserProfile,onLogout,onDeleteAccount})=>{
  ...
  if(view==="editGym"){
    const myGyms=(userProfile.gyms||[]).map(gId=>{
      ...
      const saved=(userProfile.savedGyms||[]).find(g=>g.id===gId);
      if(saved)return saved;
      // Last-resort fallback so a gym never silently disappears from the list
      return{id:gId,name:gId===userProfile.homeGym?(userProfile.homeGymName||"Your Gym"):"Registered Gym",address:gId===userProfile.homeGym?(userProfile.homeGymAddress||""):""};
    });
    return <div className="fade-in" style={{padding:"16px 20px 100px"}}>
      ...
      <h2 style={{fontSize:22,fontWeight:800,marginBottom:8}}>Home Gym</h2>
      <p style={{fontSize:13,color:T.textMuted,marginBottom:20}}>Pick your home gym from your registered gyms. Your home gym is shown to other users and is selected by default when you search for partners.</p>
      {myGyms.length===0?<Card style={{padding:20}}><p style={{fontSize:13,color:T.textDim,textAlign:"center"}}>You have no registered gyms yet.</p></Card>:
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
        {myGyms.map(g=>{const isH=userProfile.homeGym===g.id;return <Card key={g.id} onClick={()=>{setUserProfile(x=>({...x,homeGym:g.id,homeGymName:g.name,homeGymAddress:g.address||""}))}} style={{padding:14,borderColor:isH?T.accent:T.border,background:isH?T.accent+"08":T.bgCard,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
          ...
          <div style={{flex:1}}><p style={{fontWeight:600,fontSize:14}}>{g.name}</p>{g.address&&<p style={{fontSize:12,color:T.textDim,marginTop:2}}>{g.address}</p>}</div>
          {isH&&<Badge color={T.accent}>🏠 Home</Badge>}
        </Card>})}
      </div>}
      <Card style={{padding:14,background:T.bgInput,border:`1px dashed ${T.border}`}}>
        <p style={{fontSize:12,color:T.textDim,lineHeight:1.6}}>Need to add a new gym (e.g. you've relocated)? Contact support via Help & Feedback and we'll add it to your registered gyms.</p>
      </Card>
    </div>;
  }
