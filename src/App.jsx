...
// ═══ DISCOVER SCREEN ═══
// ═══ TRAINING TIPS — STACKED WEIGHT-PLATE STYLE (Discover footer) ═══
// All three visible at once, centered text, pyramid layout: small on top → large at bottom.
// Visual metaphor: weight plates stacked on a barbell.
const TipsCarousel=()=>{
  const tips=[
    // Order matters — index 0 renders smallest (top), index 2 renders largest (bottom)
  ...
  // Sizes progress smallest → largest to mimic stacked plates.
  const sizes=[
    {width:"72%",padH:14,padV:14,titleFs:13,bodyFs:11,iconFs:14,iconBox:28,gap:8},   // top plate
    {width:"86%",padH:18,padV:18,titleFs:15,bodyFs:13,iconFs:16,iconBox:34,gap:10},  // middle plate
    {width:"100%",padH:22,padV:22,titleFs:16,bodyFs:14,iconFs:18,iconBox:40,gap:12}, // bottom plate
  ];
  return <div style={{marginTop:32,paddingTop:24,borderTop:`1px solid ${T.border}30`}}>
    <p style={{fontSize:11,fontWeight:700,color:T.textDim,marginBottom:16,textTransform:"uppercase",letterSpacing:".08em",textAlign:"center"}}>💡 Training Tips</p>
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
      {tips.map((t,i)=>{const s=sizes[i];const Icon=t.icon;return <Card key={i} style={{width:s.width,padding:`${s.padV}px ${s.padH}px`,background:`linear-gradient(135deg, ${t.color}14, ${t.color}06)`,borderColor:t.color+"44",textAlign:"center",boxShadow:`0 2px 8px ${t.color}10`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:s.gap,marginBottom:8}}>
          <div style={{width:s.iconBox,height:s.iconBox,borderRadius:s.iconBox/3,background:t.color+"22",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={s.iconFs} color={t.color}/></div>
          <h4 style={{fontSize:s.titleFs,fontWeight:700,color:T.text}}>{t.title}</h4>
        </div>
        <p style={{fontSize:s.bodyFs,color:T.textMuted,lineHeight:1.6}}>{t.body}</p>
      </Card>;})}
    </div>
  </div>;
};
